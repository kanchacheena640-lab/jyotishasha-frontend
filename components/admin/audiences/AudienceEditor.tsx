...
export function CriteriaSummary({ criteria }: { criteria: SavedAudienceCriteria }) {
...
export default function AudienceEditor({ initialCriteria, audience, onClose, onSaved }: {
  initialCriteria: SavedAudienceCriteria; audience?: SavedAudience; onClose: () => void; onSaved: (audience: SavedAudience) => void;
}) {
  ...
  return <dialog ref={dialog} aria-labelledby="audience-editor-title" onCancel={e => { e.preventDefault(); if (!busy) onClose(); }} className="w-[calc(100%-2rem)] max-w-3xl rounded-xl bg-white p-0 text-gray-900 shadow-xl backdrop:bg-black/40">
    ...
    <div className="max-h-[70vh] overflow-y-auto space-y-4 p-4">
      ...
      {audience && !draft && <button disabled={busy} className={audienceButton} onClick={() => { try { setDraft(criteriaToEditor(criteria)); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "Cannot edit criteria."); } }}>Edit criteria</button>}
      {draft && <div className="space-y-4 border-t pt-3">
        ...
        <div className="flex gap-2"><button className={audienceButton} onClick={() => { try { setCriteria(audience ? editedCriteria(draft.search, draft.filters, initialCriteria, draft.booleans) : appliedFiltersToCriteria(draft.search, draft.filters)); setDraft(null); setError(""); } catch (e) { setError(e instanceof Error ? e.message : "Invalid criteria."); } }}>Apply criteria & preview</button><button className={audienceButton} onClick={() => setDraft(null)}>Cancel criteria changes</button></div>
      </div>}
