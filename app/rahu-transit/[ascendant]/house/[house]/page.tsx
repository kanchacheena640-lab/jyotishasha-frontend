...
export async function generateMetadata({
  ...
  return {
    title: `Rahu Transit in ${houseNum} House for ${asc} Ascendant – Ambition & Karma`,
    description: `Expert Vedic analysis of Rahu (North Node) transit in House ${houseNum} for ${asc} rising. Discover sudden shifts, material desires, and karmic lessons for ${currentYear}.`,
    alternates: {
...
export default async function RahuTransitHousePage({
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        "name": `How does Rahu transit in ${houseNum} house affect ${ascTitle} rising?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `For ${ascTitle} ascendant, Rahu in the ${houseNum} house amplifies desires, creates unconventional opportunities, and brings sudden karmic shifts in the life areas governed by this house.`
        }
      }
    ]
  };
    ...
    <div className="bg-gradient-to-b from-slate-900 to-indigo-950/40 py-12 md:py-20 px-4">
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-[2.5rem] px-6 md:px-16 py-16 shadow-2xl text-slate-900 relative border border-slate-100">
        ...
        <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-10 border-b border-slate-50 pb-6">
          <Link href="/rahu-transit" className="hover:text-indigo-600 transition">Rahu Hub</Link>
          <span className="text-slate-200">/</span>
          <Link href={`/rahu-transit/${params.ascendant}`} className="hover:text-indigo-600 transition">{ascTitle}</Link>
          <span className="text-slate-200">/</span>
          <span className="text-indigo-600">House {houseNum}</span>
        </nav>

        {/* H1 - Magnetic Authority */}
        <h1 className="text-4xl md:text-6xl font-black mb-8 leading-[1.1] text-slate-950 tracking-tight text-balance">
          Rahu in <span className="text-indigo-600">{houseNum} House</span> for {ascTitle} <span className="text-slate-300 font-light italic text-2xl md:text-4xl block md:inline">Rising</span>
        </h1>
        ...
        <div className="my-16 flex flex-col md:flex-row items-center gap-12 bg-indigo-50/30 p-8 rounded-[2rem] border border-indigo-100 shadow-inner">
           ...
           <div className="space-y-4">
              <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Karmic Amplification</h2>
              <p className="text-slate-600 leading-relaxed font-medium text-lg">
                In Vedic astrology, <strong>Rahu (North Node)</strong> is the insatiable seeker. Its transit in your {houseNum} house creates a <strong>magnetic pull toward expansion, obsession, and unconventional breakthroughs</strong> in this life sector.
              </p>
        ...
        <div className="space-y-8 mb-20">
          {data.sections.map((sec: any, i: number) => (
            <section key={i} className="group p-8 md:p-10 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all duration-300">
              <h3 className="text-2xl font-black mb-6 text-indigo-950 flex items-center gap-3">
                <span className="w-1.5 h-6 bg-indigo-600 rounded-full group-hover:h-10 transition-all"></span>
                {sec.heading}
              </h3>
        ...
        {/* Material vs Illusion - Strategic Contrast */}
        <div className="grid md:grid-cols-2 gap-8 mt-16 pb-20 border-b border-slate-100">
          <div className="bg-emerald-50/40 border border-emerald-100 rounded-[2.5rem] p-10">
            ...
            <h4 className="text-xl font-black mb-4 text-emerald-900 uppercase tracking-widest text-sm">Material Breakthroughs</h4>
          ...
          <div className="bg-rose-50/40 border border-rose-100 rounded-[2.5rem] p-10">
            ...
            <h4 className="text-xl font-black mb-4 text-rose-900 uppercase tracking-widest text-sm">Illusions & Risks</h4>
            <ul className="space-y-4">
        ...
        <div className="mt-20 bg-slate-950 rounded-[3.5rem] p-8 md:p-16 text-white text-center shadow-3xl relative overflow-hidden">
          ...
          <p className="text-slate-400 mb-10 max-w-2xl mx-auto text-lg md:text-xl font-medium relative z-10">
            Rahu amplifies whatever it touches. Without knowing your natal <strong>Rahu strength</strong> and dispositor dignity, these &quot;opportunities&quot; could be karmic traps. Get your precision material map.
          </p>
          
          <div className="flex flex-col md:flex-row justify-center items-center gap-6 relative z-10">
            <Link
              href={`/personalized-transit-report?planet=rahu&house=${houseNum}&ascendant=${params.ascendant}`}
              className="w-full md:w-auto bg-indigo-600 text-white font-black px-12 py-5 rounded-full hover:bg-indigo-700 transition-all shadow-2xl hover:scale-105 active:scale-95 text-lg"
            >
              Get My Rahu Reality Report →
            </Link>
            <Link
              href="/app-download"
