...
/* ---------------- Configuration & Logic ---------------- */
const RAHU_HOUSE_TRAITS: Record<number, string> = {
...
export async function generateMetadata({ params }: { params: { ascendant: string } }): Promise<Metadata> {
  ...
  return {
    title: `Rahu Transit ${currentYear} for ${ascName} Ascendant – Meaning & Effects`,
    description: `How will Rahu (North Node) transit affect ${ascName} Rising? Expert Vedic analysis of karmic shifts, material desires, and sudden breakthroughs for ${currentYear}.`,
    alternates: { canonical: `https://www.jyotishasha.com/rahu-transit/${a}` },
  };
}

/* ---------------- Page Component ---------------- */
export default async function RahuTransitAscendantPage({ params, searchParams }: { params: { ascendant: string }; searchParams?: { lang?: string; house?: string }; }) {
  ...
  const initialData = await fetchTransitContent({ ascendant, planet: "rahu", house: initialHouse, lang });

  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: [
      { "@type": "Question", "name": `How does Rahu transit affect ${ascName} Rising?`, "acceptedAnswer": { "@type": "Answer", "text": `For ${ascName} Ascendant, Rahu moves through ${currentRashi}, influencing the ${currentHouse} house of karma and desires.` } }
    ]
  };
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-[2.5rem] px-6 md:px-12 py-16 shadow-2xl text-slate-900 border border-slate-100 overflow-hidden relative">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 border-b pb-6">
          <h1 className="text-3xl md:text-5xl font-black leading-tight text-slate-950 tracking-tight">
            Rahu Transit {currentYear} for <span className="text-indigo-600">{ascName}</span> <span className="text-slate-550 font-light italic text-2xl md:text-3xl block md:inline">Rising</span>
          </h1>
          <Link href="/rahu-transit" className="text-sm font-bold text-indigo-700 uppercase tracking-widest hover:underline flex items-center gap-1">
            ← Global Hub
          </Link>
        </header>
        ...
        <div className="my-12 flex justify-center">
          <DynamicTransitChart ascendant={ascendant} activePlanet="rahu" house={currentHouse} size={340} />
        </div>

        {/* Intro Section with Dynamic Rahu Traits */}
        <div className="prose prose-slate max-w-none mb-10">
          <p className="text-lg text-slate-700 leading-relaxed font-medium mb-4">
            Rahu transit (North Node Gochar) through {currentRashi} represents a phase of <strong>karmic expansion and obsession</strong> for {ascName} Ascendant.
          </p>

          <h2 className="text-2xl font-bold text-indigo-900 mb-4">
            The North Node Shift: {previousRashi} → {currentRashi}
          </h2>

          <p className="text-lg text-slate-700 leading-relaxed font-medium">
            Rahu, the planet of amplification, moves from your {previousHouse} house to the <strong>{currentHouse} house</strong>. 

            Your material focus shifts from <strong>{RAHU_HOUSE_TRAITS[previousHouse]}</strong> toward the themes of <strong>{RAHU_HOUSE_TRAITS[currentHouse]}</strong>. This transit often brings sudden breakthroughs if handled with clarity.
          </p>
        </div>
        ...
        <section className="bg-slate-900 rounded-[2rem] p-8 mb-16 text-white shadow-xl relative overflow-hidden border border-white/5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm relative z-10 font-medium">
            ...
            <div>
              ...
              <p className="text-lg text-amber-400 italic font-bold">Retrograde</p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-20 border-t pt-12">
          <h2 className="text-3xl font-black mb-8 text-slate-950">
            {ascName} Material Forecast for Every House
          </h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => (
              ...
              >
                <p className="text-[10px] font-black text-indigo-600 uppercase mb-1 tracking-tighter">View Focus</p>
