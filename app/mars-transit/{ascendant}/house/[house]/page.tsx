...
export async function generateMetadata({
  ...
  return {
    title: `Mars Transit in ${asc} Ascendant – House ${houseNum} Effects | Jyotishasha`,
    description: `Mars transit effects in House ${houseNum} for ${asc} ascendant as per Vedic astrology.`,
    alternates: {
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-2xl px-6 md:px-10 py-14 shadow-xl text-black">
        ...
        <div className="space-y-10">
          {data.sections.map((sec: any, i: number) => (
            <section key={i} className="border-l-4 border-red-700 pl-6">
        ...
        <div className="mt-12 bg-gradient-to-r from-red-600 to-orange-600 rounded-2xl p-7 text-white shadow-lg">
          <h3 className="text-2xl font-bold mb-3">
            This Is a General Mars Transit — Want Your Real Prediction?
          </h3>

          <p className="text-white/95 leading-relaxed mb-5">
            This page explains <strong>Mars transit in House {houseNum}</strong> for
            <strong> {ascTitle} Ascendant</strong>.
            <br /><br />
            Your real results depend on:
          </p>

          <ul className="list-disc pl-6 space-y-1 text-white/95 mb-6">
            <li>Which houses Mars rules in your birth chart</li>
            <li>Mangal Dosha, combustion & exact degree strength</li>
            <li>Aspects from Saturn, Jupiter, Rahu–Ketu</li>
            <li>Your running Mahadasha & Antardasha</li>
            <li>Moon sign reaction to Mars activation</li>
          </ul>

          <a
            href={`/personalized-transit-report?planet=mars&house=${houseNum}&ascendant=${params.ascendant}`}
            className="inline-block bg-white text-red-700 font-bold px-8 py-4 rounded-xl hover:bg-gray-100 transition"
          >
            Get My Personalized Mars Transit Report →
          </a>
        </div>
