import type { Metadata } from "next";
import { notFound } from "next/navigation";

const BACKEND = "https://jyotishasha-backend.onrender.com";

/* ---------------- DATA ---------------- */
async function fetchTransit(
  ascendant: string,
  planet: string,
  house: number
) {
  const res = await fetch(
    `${BACKEND}/api/transit?ascendant=${ascendant}&planet=${planet}&house=${house}&lang=en`,
    { cache: "no-store" }
  );
  if (!res.ok) return null;
  return res.json();
}

/* ---------------- SEO ---------------- */
export async function generateMetadata({
  params,
}: {
  params: {
    "planet-transit": string;
    ascendant: string;
    house: string;
  };
}): Promise<Metadata> {
  const houseNum = Number(params.house);
  if (!houseNum || houseNum < 1 || houseNum > 12) {
    return { robots: { index: false, follow: false } };
  }

  const planet = params["planet-transit"].replace("-transit", "");
  const planetTitle = planet.charAt(0).toUpperCase() + planet.slice(1);

  const asc =
    params.ascendant.charAt(0).toUpperCase() +
    params.ascendant.slice(1);

  return {
    title: `${planetTitle} Transit in ${asc} Ascendant – House ${houseNum} Effects | Jyotishasha`,
    description: `${planetTitle} transit effects in House ${houseNum} for ${asc} ascendant as per Vedic astrology.`,
    alternates: {
      canonical: `https://www.jyotishasha.com/${params["planet-transit"]}/${params.ascendant}/house-${houseNum}`,
    },
  };
}

/* ---------------- PAGE ---------------- */
export default async function TransitHousePage({
  params,
}: {
  params: {
    "planet-transit": string;
    ascendant: string;
    house: string;
  };
}) {
  const houseNum = Number(params.house);
  if (!houseNum || houseNum < 1 || houseNum > 12) notFound();

  const planet = params["planet-transit"].replace("-transit", "");

  const data = await fetchTransit(
    params.ascendant,
    planet,
    houseNum
  );

  if (!data) notFound();

  return (
    <div className="bg-gradient-to-b from-slate-900 to-slate-800 py-16">
      <article className="max-w-5xl mx-auto bg-white rounded-2xl px-6 md:px-10 py-14 shadow-xl text-black">

        {/* TITLE */}
        <h1 className="text-3xl md:text-4xl font-bold mb-6">
          {data.title}
        </h1>

        {/* SUMMARY */}
        <p className="text-gray-800 text-lg leading-relaxed mb-12">
          {data.summary}
        </p>

        {/* SECTIONS */}
        <div className="space-y-10">
          {data.sections.map((sec: any, i: number) => (
            <section key={i} className="border-l-4 border-indigo-600 pl-6">
              <h2 className="text-2xl font-semibold mb-3">
                {sec.heading}
              </h2>
              <ul className="list-disc pl-6 space-y-1 text-gray-700">
                {sec.points.map((p: string, j: number) => (
                  <li key={j}>{p}</li>
                ))}
              </ul>
            </section>
          ))}
        </div>

        {/* STRENGTHS & CHALLENGES */}
        <div className="grid md:grid-cols-2 gap-8 mt-14">
          <div className="bg-green-50 rounded-xl p-6">
            <h3 className="text-xl font-semibold mb-3 text-green-800">
              Strengths
            </h3>
            <ul className="list-disc pl-6 space-y-1 text-gray-700">
              {data.strengths.map((s: string, i: number) => (
                <li key={i}>{s}</li>
              ))}
            </ul>
          </div>

          <div className="bg-red-50 rounded-xl p-6">
            <h3 className="text-xl font-semibold mb-3 text-red-800">
              Challenges
            </h3>
            <ul className="list-disc pl-6 space-y-1 text-gray-700">
              {data.challenges.map((c: string, i: number) => (
                <li key={i}>{c}</li>
              ))}
            </ul>
          </div>
        </div>

        {/* CLOSING */}
        <div className="mt-12 border-t pt-6">
          <p className="text-gray-800 text-lg">
            <strong>Closing Insight:</strong> {data.closing}
          </p>
        </div>

        {/* CTA – PERSONALIZED */}
        <div className="mt-12 bg-gradient-to-r from-amber-500 to-orange-600 rounded-2xl p-8 text-white">
          <h3 className="text-2xl font-bold mb-3">
            Want Your Personalized Transit Prediction?
          </h3>

          <p className="mb-5 leading-relaxed">
            This is a <strong>general {planet} transit</strong> for
            <strong> {params.ascendant} ascendant</strong>.
            Real results depend on your exact chart, aspects,
            planetary strength and running dasha.
          </p>

          <a
            href={`/personalized-transit-report?planet=${planet}&house=${houseNum}&ascendant=${params.ascendant}`}
            className="inline-block bg-white text-orange-700 font-bold px-8 py-4 rounded-xl hover:bg-gray-100 transition"
          >
            Get My Personalized Report →
          </a>
        </div>

      </article>
    </div>
  );
}
