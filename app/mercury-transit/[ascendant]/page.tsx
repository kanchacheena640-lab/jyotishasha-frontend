...
export async function generateMetadata({
  ...
  return {
    title: `Mercury Transit for ${asc} Ascendant – House-wise Effects | Jyotishasha`,
    description: `House-wise effects of Mercury transit for ${asc} ascendant as per Vedic astrology.`,
    alternates: {
    ...
    <div className="bg-gradient-to-b from-emerald-900 to-emerald-800 py-16">
      <article className="max-w-5xl mx-auto bg-white rounded-2xl px-6 md:px-10 py-14 shadow-xl text-black">
        ...
        <p className="text-gray-800 mb-8">
          This page explains <strong>house-wise Mercury transit effects</strong> for
          <strong> {titleCase(ascendant)} Ascendant</strong> focusing on intellect,
          communication, learning, business and decision-making.
        </p>
        ...
        <section className="mt-12 border-t pt-6">
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            {ASCENDANTS.map((a) => (
              <Link
                key={a}
                href={`/mercury-transit/${a}`}
                className={`hover:underline ${
                  a === ascendant ? "text-gray-400 pointer-events-none" : "text-emerald-700"
                }`}
              >
                {titleCase(a)}
              </Link>
