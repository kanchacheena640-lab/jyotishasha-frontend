...
export async function generateMetadata({
  ...
  if (!a || !isValidAscendant(a)) {
    return {
      title: "Ketu Transit Ascendant Effects | Jyotishasha",
  ...
  return {
    title: `Ketu Transit for ${ascName} Ascendant – House-wise Effects | Jyotishasha`,
    description: `Detailed house-wise effects of Ketu transit for ${ascName} ascendant as per Vedic astrology.`,
    alternates: {
      canonical: `https://www.jyotishasha.com/ketu-transit/${a}`,
    },
    openGraph: {
      title: `Ketu Transit for ${ascName} Ascendant`,
      description: `House-wise effects of Ketu transit for ${ascName} ascendant.`,
      url: `https://www.jyotishasha.com/ketu-transit/${a}`,
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-2xl px-6 md:px-10 py-14 shadow-xl text-black">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-3xl md:text-4xl font-bold">
            Ketu Transit Effects for {titleCase(ascendant)} Ascendant
          </h1>
          <Link href="/ketu-transit" className="text-sm text-gray-700 hover:underline">
            ← Back to Ketu Transit
          </Link>
        </div>

        <p className="text-gray-800 mb-8 leading-relaxed">
          This page explains the <strong>house-wise effects</strong> of Ketu transit for{" "}
          <strong>{titleCase(ascendant)} Ascendant</strong>. Ketu represents detachment,
          past-life karma, spiritual awakening, isolation and inner transformation.
        </p>

        {/* Snapshot */}
        <section className="bg-gray-900 rounded-xl p-6 mb-10 text-white">
          <h2 className="text-xl font-semibold mb-4">Current Ketu Transit Snapshot</h2>
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            <div><strong>Planet:</strong> Ketu</div>
            <div><strong>Current Rashi:</strong> {ketuPos?.rashi || "-"}</div>
            <div><strong>Degree:</strong> {ketuPos?.degree ? `${ketuPos.degree}°` : "-"}</div>
        ...
        <section className="mt-14">
          <h2 className="text-2xl font-semibold mb-4">
            Ketu Transit in {titleCase(ascendant)} Ascendant – House-wise Effects
          </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => (
              <Link
                ...
                className="border rounded-xl px-4 py-3 text-sm hover:border-gray-700 hover:bg-gray-50 transition"
              >
                <strong>House {h}</strong>
                <div className="text-gray-600 mt-1">
                  Ketu transit effects in House {h}
                </div>
              </Link>
            ))}
          </div>
