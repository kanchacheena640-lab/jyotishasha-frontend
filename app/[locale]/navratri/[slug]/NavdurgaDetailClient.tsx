...
export default function NavdurgaDetailClient({
  ...
  const goddessName = content.title
    .split(":")[0]
    .replace("Maa ", "")
    .replace("माँ ", "")
  ...
  return (
    <section className="max-w-5xl mx-auto px-4 py-12">
      ...
      <p className="mt-4 mb-10 text-gray-700 max-w-4xl mx-auto text-center leading-relaxed">
        ...
        {lang === "hi" ? (
          <>
            {navType === "chaitra" ? "चैत्र" : "शारदीय"} नवरात्रि {year} में
            माँ {goddessName} की पूजा {formatDate(dayData?.date)} को की जाती है।
          </>
        ) : (
          <>
            During {navType === "chaitra" ? "Chaitra" : "Shardiya"} Navratri {year},
            Maa {goddessName} is worshipped on {formatDate(dayData?.date)}.
          </>
        )}
      ...
      <div className="mt-6 mb-16 text-center">
        ...
        <h2 className="text-xl font-semibold text-[#7A1C1C] mb-6">
          {lang === "hi" ? "अन्य देवी रूप देखें" : "Explore Other Forms of Maa Durga"}
        </h2>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {NAVDURGA_LIST
            ...
            .map((m) => (
              <a
                key={m.slug}
                href={`/${lang === "hi" ? "hi/" : ""}navratri/${m.slug}`}
          ...
          {section.content && (
            <p className="text-gray-700 leading-relaxed mb-4">
              {section.content
                ...
                .replace("{navType}", lang === "hi"
                  ? (navType === "chaitra" ? "चैत्र" : "शारदीय")
                  : (navType === "chaitra" ? "Chaitra" : "Shardiya"))
