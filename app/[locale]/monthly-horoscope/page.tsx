...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE_URL}${langPath}/monthly-horoscope`;

  const title = lang === "hi"
    ? "मासिक राशिफल - सभी 12 राशियाँ | ज्योतिष आशा"
    : "Monthly Horoscope - All 12 Zodiac Signs | Jyotishasha";

  const description = lang === "hi"
    ? "अपनी राशि चुनें और इस महीने का राशिफल पढ़ें - करियर, प्रेम, धन और स्वास्थ्य की भविष्यवाणी।"
    : "Select your zodiac sign and read this month's horoscope - career, love, finance and health predictions.";
...
export default async function MonthlyHoroscopeIndexPage({ params }: PageProps) {
  const { locale } = await params;
  const lang = locale === "hi" ? "hi" : "en";
  const langPath = lang === "hi" ? "/hi" : "";
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      {/* HERO */}
      <section className="mb-12 rounded-3xl bg-gradient-to-r from-purple-700 to-indigo-700 px-8 py-12 text-white text-center">
        <h1 className="text-4xl md:text-5xl font-extrabold mb-4">
          {lang === "hi" ? "मासिक राशिफल" : "Monthly Horoscope"}
        </h1>
        <p className="text-purple-100 text-lg max-w-2xl mx-auto">
          {lang === "hi"
            ? "अपनी राशि चुनें और जानें इस महीने करियर, प्रेम, धन और स्वास्थ्य पर ग्रहों का प्रभाव।"
            : "Select your zodiac sign to read this month's horoscope for career, love, finance and health."}
        </p>
      </section>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {ZODIACS.map((z) => {
          ...
          return (
            <Link
              key={z.sign}
              href={`${langPath}/monthly-horoscope/${z.sign}`}
        ...
        <div className="rounded-2xl border bg-white p-6 hover:shadow-md transition">
          ...
          <p className="text-sm text-gray-600 mb-5">
            {lang === "hi"
              ...
              : "Get today's horoscope with love, career and planetary guidance."}
          </p>
          <Link
            href={`${langPath}/daily-horoscope`}
            className="inline-block rounded-xl border border-blue-300 px-5 py-2.5 font-semibold text-gray-900 hover:bg-blue-50 transition"
          >
