...
async function fetchTransit(ascendant: string, house: number, lang: "en" | "hi") {
  const res = await fetch(
    `${BACKEND}/api/transit?ascendant=${ascendant}&planet=saturn&house=${house}&lang=${lang}`,
...
export async function generateMetadata({
  ...
  return {
    title: isHi
      ? `शनि गोचर ${currentYear} ${houseNum}वें भाव में ${asc} लग्न के लिए – कर्म और अनुशासन`
      : `Saturn Transit ${currentYear} in ${houseNum} House for ${asc} Rising – Karma & Discipline`,
    description: isHi
      ? `${asc} लग्न के लिए ${houseNum}वें भाव में शनि गोचर ${currentYear} का सरल विश्लेषण – मेहनत, जिम्मेदारी और लंबे समय की सफलता।`
      : `Simple Vedic analysis of Saturn transit ${currentYear} in House ${houseNum} for ${asc} Rising – karmic lessons, discipline, and long-term mastery.`,
    alternates: {
...
export default async function SaturnTransitHousePage({
  ...
  const faqText = isHi
    ? `${ascTitle} लग्न के लिए ${houseNum}वें भाव में शनि से मेहनत और जिम्मेदारी बढ़ती है।`
    : `For ${ascTitle} ascendant, Saturn in the ${houseNum} house brings discipline and long-term restructuring.`;
    ...
    <div className="bg-gradient-to-b from-slate-950 to-slate-900 py-12 md:py-20 px-4">
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-[2.5rem] px-6 md:px-16 py-16 shadow-2xl text-slate-900 relative border border-slate-100">
        ...
        <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-10 border-b border-slate-50 pb-6">
          <Link href={`/${isHi ? "hi/" : ""}saturn-transit`} className="hover:text-slate-600 transition">
            {isHi ? "शनि हब" : "Saturn Hub"}
          </Link>
          <span className="text-slate-200">/</span>
          <Link href={`/${isHi ? "hi/" : ""}saturn-transit/${ascendant}`} className="hover:text-slate-600 transition">
            {ascTitle}
          </Link>
          <span className="text-slate-200">/</span>
        ...
        <h1 className="text-4xl md:text-6xl font-black mb-8 leading-[1.1] text-slate-950 tracking-tight">
          {isHi ? (
            <>
              शनि {houseNum}वें भाव में {ascTitle} लग्न के लिए –{" "}
              <span className="text-slate-600">कर्म और अनुशासन</span>
            </>
          ) : (
            <>
              Saturn in <span className="text-slate-600">{houseNum} House</span> for {ascTitle}{" "}
        ...
        <div className="my-16 flex flex-col md:flex-row items-center gap-12 bg-slate-50 p-8 rounded-[2rem] border border-slate-200 shadow-inner">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">
              {isHi ? "अब नींव मज़बूत करने का समय" : "Current House Activation"}
            </h2>
            <p className="text-slate-600 leading-relaxed font-medium text-lg">
              {isHi ? (
                <>वैदिक ज्योतिष में <strong>शनि</strong> कर्म का मालिक है। आपके {houseNum}वें भाव में आने से <strong>मेहनत, जिम्मेदारी और लंबे समय की सफलता</strong> का दौर शुरू होता है।</>
              ) : (
                <>In Vedic astrology, Saturn is the Lord of Karma. Its transit in your {houseNum} house initiates a <strong>cycle of discipline, heavy responsibility, and permanent foundations</strong>.</>
              )}
            </p>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-200 text-slate-700 rounded-full text-xs font-black uppercase tracking-widest border border-slate-300">
              {isHi ? "अब मेहनत का फल मिलने का दौर" : "Status: Mastery Phase"}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8 mb-20">
          {data.sections?.map((sec: any, i: number) => (
            <section
              key={i}
              className="group p-8 md:p-10 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-slate-300 transition-all duration-300"
            >
              <h3 className="text-2xl font-black mb-6 text-slate-900 flex items-center gap-3">
                <span className="w-1.5 h-6 bg-slate-900 rounded-full group-hover:h-8 transition-all"></span>
                {sec.heading}
              </h3>
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-16 pb-20 border-b border-slate-100">
          <div className="bg-emerald-50/50 border border-emerald-100 rounded-[2rem] p-10">
            <div className="w-12 h-12 bg-emerald-100 rounded-2xl flex items-center justify-center mb-6 text-emerald-600 text-2xl">🏛️</div>
        ...
        <div className="mt-20 bg-slate-950 rounded-[3.5rem] p-8 md:p-16 text-white text-center shadow-3xl relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_right,_var(--tw-gradient-stops))] from-slate-600/30 via-transparent to-transparent opacity-50"></div>
          ...
          <p className="text-slate-400 mb-10 max-w-2xl mx-auto text-lg md:text-xl font-medium relative z-10">
            {isHi ? (
              <>शनि 2.5 साल रुकता है। बिना अपनी कुंडली में शनि की ताकत, दृष्टि और दशा के पता चलाए मेहनत बेकार जा सकती है। अपना लंबा प्लान ले लो।</>
            ) : (
              <>Saturn stays for 2.5 years. Without knowing your natal Shani strength, its aspects, and your Mahadasha cycle, you might be fighting a battle you aren’t meant to win yet. Get your long-term mastery audit.</>
            )}
          </p>

          <div className="flex flex-col md:flex-row justify-center items-center gap-6 relative z-10">
            <Link
              href={`/${isHi ? "hi/" : ""}personalized-transit-report?planet=saturn&house=${houseNum}&ascendant=${params.ascendant}`}
            ...
            >
              {isHi ? "मेरा कर्म ऑडिट रिपोर्ट लो →" : "Get My Karmic Audit Report →"}
            </Link>
            <Link
              href="/app-download"
            ...
            >
              {isHi ? "रोज़ाना शनि उपाय" : "Daily Shani Remedies"}
            </Link>
          </div>
        </div>
      </article>
