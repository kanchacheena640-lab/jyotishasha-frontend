...
async function fetchTransit(ascendant: string, house: number, lang: "en" | "hi") {
  const res = await fetch(
    `${BACKEND}/api/transit?ascendant=${ascendant}&planet=mars&house=${house}&lang=${lang}`,
...
export async function generateMetadata({
  ...
  return {
    title: isHi
      ? `मंगल गोचर ${currentYear} ${houseNum}वें भाव में ${asc} लग्न के लिए – ऊर्जा और साहस`
      : `Mars Transit ${currentYear} in ${houseNum} House for ${asc} Rising – Energy & Courage`,
    description: isHi
      ? `${asc} लग्न के लिए ${houseNum}वें भाव में मंगल गोचर ${currentYear} का सरल विश्लेषण – जोश, महत्वाकांक्षा और सावधानी।`
      : `Simple Vedic analysis of Mars transit ${currentYear} in House ${houseNum} for ${asc} Rising – energy, ambition, conflicts, and action.`,
    alternates: {
...
export default async function MarsTransitHousePage({
  ...
  const faqText = isHi
    ? `${ascTitle} लग्न के लिए ${houseNum}वें भाव में मंगल से जोश और साहस बढ़ता है।`
    : `For ${ascTitle} ascendant, Mars in the ${houseNum} house brings intense drive and willpower.`;
    ...
    <div className="bg-gradient-to-b from-slate-900 to-red-950/20 py-12 md:py-20 px-4">
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-[2.5rem] px-6 md:px-16 py-16 shadow-2xl text-slate-900 relative border border-slate-100">
        ...
        <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-10 border-b border-slate-50 pb-6">
          <Link href={`/${isHi ? "hi/" : ""}mars-transit`} className="hover:text-red-600 transition">
            {isHi ? "मंगल हब" : "Mars Hub"}
          </Link>
          <span className="text-slate-200">/</span>
          <Link href={`/${isHi ? "hi/" : ""}mars-transit/${ascendant}`} className="hover:text-red-600 transition">
            {ascTitle}
          </Link>
          <span className="text-slate-200">/</span>
        ...
        <h1 className="text-4xl md:text-6xl font-black mb-8 leading-[1.1] text-slate-950 tracking-tight">
          {isHi ? (
            <>
              मंगल {houseNum}वें भाव में {ascTitle} लग्न के लिए –{" "}
              <span className="text-red-600">ऊर्जा और साहस</span>
            </>
          ) : (
            <>
              Mars in <span className="text-red-600">{houseNum} House</span> for {ascTitle}{" "}
        ...
        <div className="my-16 flex flex-col md:flex-row items-center gap-12 bg-red-50/30 p-8 rounded-[2rem] border border-red-100 shadow-inner">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">
              {isHi ? "अब जोश का दौर" : "Current House Activation"}
            </h2>
            <p className="text-slate-600 leading-relaxed font-medium text-lg">
              {isHi ? (
                <>वैदिक ज्योतिष में <strong>मंगल</strong> योद्धा ग्रह है। आपके {houseNum}वें भाव में आने से <strong>जोश, साहस और काम करने की ताकत</strong> बढ़ जाती है।</>
              ) : (
                <>In Vedic astrology, Mars is the warrior planet. Its transit in your {houseNum} house brings a surge in <strong>willpower, initiative, and the ability to conquer obstacles</strong>.</>
              )}
            </p>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-red-100 text-red-700 rounded-full text-xs font-black uppercase tracking-widest">
              {isHi ? "अब जोश का समय है" : "Status: High Energy Phase"}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8 mb-20">
          {data.sections?.map((sec: any, i: number) => (
            <section
              key={i}
              className="group p-8 md:p-10 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-red-100 transition-all duration-300"
            >
              <h3 className="text-2xl font-black mb-6 text-blue-950 flex items-center gap-3">
                <span className="w-1.5 h-6 bg-red-600 rounded-full group-hover:h-8 transition-all"></span>
                {sec.heading}
              </h3>
        ...
        <div className="mt-20 bg-slate-950 rounded-[3.5rem] p-8 md:p-16 text-white text-center shadow-3xl relative overflow-hidden">
          ...
          <h3 className="text-3xl md:text-5xl font-black mb-6 relative z-10 leading-tight">
            {isHi ? (
              <>अब जोश को सही दिशा दो!</>
            ) : (
              <>Stop Guessing. Take Command of Your Karma.</>
            )}
          </h3>
          <p className="text-slate-400 mb-10 max-w-2xl mx-auto text-lg md:text-xl font-medium relative z-10">
            {isHi ? (
              <>मंगल ताकत है, लेकिन बिना अपनी कुंडली और दशा के पता चलाए काम करने से थकान हो सकती है। अपना सही एक्शन प्लान ले लो।</>
            ) : (
              <>Mars can be a weapon or a tool. Without knowing your natal Mangal strength and current Dasha, action can lead to burnout. Get your precision action map.</>
            )}
          </p>

          <div className="flex flex-col md:flex-row justify-center items-center gap-6 relative z-10">
            <Link
              href={`/${isHi ? "hi/" : ""}personalized-transit-report?planet=mars&house=${houseNum}&ascendant=${params.ascendant}`}
            ...
            >
              {isHi ? "मेरा एक्शन रिपोर्ट लो →" : "Get My Action Report →"}
            </Link>
            <Link
              href="/app-download"
            ...
            >
              {isHi ? "रोज़ाना ऊर्जा अलर्ट" : "Daily Energy Alerts"}
            </Link>
          </div>
        </div>
      </article>
