    ...
    <div className="bg-gradient-to-b from-slate-900 to-amber-950/20 py-12 md:py-20 px-4">
      ...
      <article className="max-w-5xl mx-auto bg-white rounded-[2.5rem] px-6 md:px-16 py-16 shadow-2xl text-slate-900 relative border border-slate-100">
        ...
        <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-10 border-b border-slate-50 pb-6">
          <Link href={`/${isHi ? "hi/" : ""}sun-transit`} className="hover:text-amber-600 transition">
            {isHi ? "सूर्य हब" : "Sun Hub"}
          </Link>
          <span className="text-slate-200">/</span>
          <Link href={`/${isHi ? "hi/" : ""}sun-transit/${ascendant}`} className="hover:text-amber-600 transition">
            {ascTitle}
          </Link>
          <span className="text-slate-200">/</span>
        ...
        <h1 className="text-4xl md:text-6xl font-black mb-8 leading-[1.1] text-slate-950 tracking-tight">
          {isHi ? (
            <>
              सूर्य {houseNum}वें भाव में {ascTitle} लग्न के लिए –{" "}
              <span className="text-amber-600">शक्ति और पहचान</span>
            </>
          ) : (
            <>
              Sun in <span className="text-amber-600">{houseNum} House</span> for {ascTitle}{" "}
        ...
        <div className="my-16 flex flex-col md:flex-row items-center gap-12 bg-amber-50/40 p-8 rounded-[2rem] border border-amber-100 shadow-inner">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">
              {isHi ? "अब चमकने और आगे बढ़ने का दौर" : "Current House Activation"}
            </h2>
            <p className="text-slate-600 leading-relaxed font-medium text-lg">
              {isHi ? (
                <>वैदिक ज्योतिष में <strong>सूर्य</strong> आत्मा और राजा का ग्रह है। आपके {houseNum}वें भाव में आने से <strong>आत्मविश्वास, पहचान और नेतृत्व की ताकत</strong> बढ़ जाती है।</>
              ) : (
                <>In Vedic astrology, the Sun is the Soul and the King. Its transit in your {houseNum} house acts as a spotlight, bringing <strong>clarity, leadership opportunities, and a surge in personal vitality</strong>.</>
              )}
            </p>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-100 text-amber-700 rounded-full text-xs font-black uppercase tracking-widest border border-amber-200">
              {isHi ? "अब सबकी नज़रों में आने का समय" : "Status: High Visibility Phase"}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8 mb-20">
          {data.sections?.map((sec: any, i: number) => (
            ...
            >
              <h3 className="text-2xl font-black mb-6 text-amber-950 flex items-center gap-3">
                <span className="w-1.5 h-6 bg-amber-500 rounded-full group-hover:h-8 transition-all"></span>
                {sec.heading}
              </h3>
        ...
        <div className="grid md:grid-cols-2 gap-8 mt-16 pb-20 border-b border-slate-100">
          <div className="bg-amber-50/50 border border-amber-100 rounded-[2rem] p-10">
          ...
          <div className="bg-rose-50/30 border border-rose-100 rounded-[2rem] p-10">
        ...
        <div className="mt-20 bg-slate-950 rounded-[3.5rem] p-8 md:p-16 text-white text-center shadow-3xl relative overflow-hidden">
          ...
          <p className="text-slate-400 mb-10 max-w-2xl mx-auto text-lg md:text-xl font-medium relative z-10">
            {isHi ? (
              <>सूर्य 1 महीना रहता है। बिना अपनी कुंडली में सूर्य की ताकत और दशा के पता चलाए तुम गलत रास्ते पर चल सकते हो। अपना सही सोलर मैप ले लो।</>
            ) : (
              <>The Sun stays for 30 days. Without knowing your natal Surya strength, its lordship, and your Dasha cycle, you might be following a path that doesn’t align with your soul. Get your precision solar audit.</>
            )}
          </p>

          <div className="flex flex-col md:flex-row justify-center items-center gap-6 relative z-10">
            <Link
              href={`/${isHi ? "hi/" : ""}personalized-transit-report?planet=sun&house=${houseNum}&ascendant=${params.ascendant}`}
            ...
            >
              {isHi ? "मेरा सूर्य रिपोर्ट लो →" : "Get My Solar Prediction →"}
            </Link>
            <Link
              href="/app-download"
            ...
            >
              {isHi ? "रोज़ाना सूर्य अंतर्दृष्टि" : "Daily Solar Insights"}
            </Link>
          </div>
        </div>
      </article>
